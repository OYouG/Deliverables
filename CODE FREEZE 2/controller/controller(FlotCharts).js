    ...
    successCallbackWVTSMS : function(start, end, webVisits, ticketSales, comSpend) {
        ...
        for (var i = 0; i < trackSpend.length; i++){
	        ...
	        for (var j = 0; j < dateArray.length; j++){
		        if(item[1] == "TV - :15" || item [1] == "TV - :30 & :15"){
			        spend.push([dateArray[j]*1,dailySpend]);
		        }
		        else{
			        radioSpend.push([dateArray[j]*1, dailySpend])
		        }
        ...
        if (start == null || start == ''){
            ...
            for (i=0; i<radioSpend.length; i++){
	            var item = radioSpend[i];
		   		var tempDate = item[0];
		   		if ((tempDate >= gd(startSplit[2], startSplit[0], startSplit[1])) && (tempDate <= gd(endSplit[2], endSplit[0], endSplit[1]))){

                	finalRadioSpend.push(item);
            	}
            }
        }
...
*/
	        ...
	        label: "Media Spend - Public Radio",
            data: finalRadioSpend,
			yaxis:2,
			stack: true,
/*
            points:{
...
*/
                ...
                yaxes: [
                    ...
                    {position:"right",
	                 axisLabel:"Website Visits/Media Spend (in $)",
	                 max: 50000
                    }
                    ],
                grid:{
               ...
               tooltipOpts: {
                content:function (label, x, y) {
                    ...
                    if (label == "Website Visits"){
						...
						tooltip += '<li>Date: ' + date.toLocaleDateString() + '</li>';
						tooltip += '<li>Website Visits: ' + y.toLocaleString() + '</li></ul>';
                    } else if (label == "Media Spend - Broadcast" || label == "Media Spend - Public Radio"){
...
*/
    ...
    successCallbackTSMS : function(start, end, ticketSales, comSpend) {
        ...
        var options = {
                ...
                yaxes: [
                    {position:"left",
	                    axisLabel:"Ticket Gross/Total Media Spend (in $)"
                    },
                    {position:"right"}
                    ],
                grid:{
...
//             }
        ...
        var options = {
               ...
               tooltipOpts: {
                content:function (label, x, y) {
                    ...
                    var tooltip = '<h3>' + label + '</h3><ul>';
                    tooltip += '<li>Date is ' + date.toLocaleDateString() + '</li>';
                    tooltip += '<li>Website Visits: ' + y + '</li></ul>';
                    return tooltip;
                },
                shifts: {
                    x: 10,
   ...
   renderCIAdsGraph : function(start, end, data) {
    ...
    successCallbackCIAdsLine : function(start, end, ciData) {
		...
		var newTicks = [];
        for(var i = 0; i < finalClick.length; i++){
            newTicks.push(finalClick[i][0]);
        }
            ...
            series: {stack: 0,
               ...
               tooltipOpts: {
                content:function (label, x, y) {
                    ...
                    var tooltip = '<h3>' + label + '</h3><ul>';
                    tooltip += '<li>Date is ' + date.toLocaleDateString() + '</li>';
                    tooltip += '<li>Ticket Gross: $' + y + '</li></ul>';
                    return tooltip;
                },
                shifts: {
                    x: 10,
   ...
   renderTPSGraph : function(start, end, data) {
    ...
    successCallbackTPSGraph : function(start, end, data) {
        ...
        function gdHour(date, time) {
		    ...
		    var year = "20"+splitDate[2];
			console.log(time);
			var thisDate = new Date(parseInt(year), parseInt(splitDate[0]) - 1, parseInt(splitDate[1]), parseInt(time)+12);
			console.log(thisDate);
            return thisDate.getTime()-tzOffset;
        }
        function gd(date) {
            var splitDate = date.split("/");
            var year = splitDate[2];
            ...
            return new Date(parseInt(year), parseInt(splitDate[0]) - 1, parseInt(splitDate[1])).getTime()-tzOffset;
        }
        function parseDate(start, end, data, parsedData){
            for(var i = 0; i < data.length; i++){
                var item = data[i];
        ...
        var options = {
               ...
               tooltipOpts: {
                content:function (label, x, y) {
                    var date = new Date(+x);
                    var tooltip = '<h3>' + label + '</h3><ul>';
                    tooltip += '<li>Date of Show: ' + date.toLocaleDateString() + '</li>';
                    tooltip += '<li>Ticket Amount: ' + y + '</li></ul>';
                    return tooltip;
                },
                shifts: {
                    x: 10,
   ...
   renderSeatingGraph : function(start, end, data) {
    ...
    successCallbackSeating : function(start, end, scData) {
      ...
      renderOrch1Graph : function(start, end, data) {
        var callback = this.successCallbackOrch1;
        $("#Orch1Form").submit(function(e) {
            e.preventDefault();
            callback($("#orch1StartDate").val(), $("#orch1EndDate").val(), data);
        });
        this.successCallbackOrch1(start, end, data);
   },
   
   successCallbackOrch1 : function(start, end, plData){
       var orch1 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].ORCH1Sold;
            orch1 += parseInt(item);
            counter += 1;
            var cap = counter * 322;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                var item = plData[i];
                var date = gd(item.EventDate);
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 322;
                    return result += parseInt(item.ORCH1Sold);
                }
            }
        }
        ...
        if (start == null || start == ''){
            ...
            parsedData = parseDate(start, end);
        }
       ...
       var plotObj = $.plot($("#flot-pie-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
   ...
   renderOrch2Graph : function(start, end, data) {
        var callback = this.successCallbackOrch2;
        $("#Orch2Form").submit(function(e) {
            e.preventDefault();
            callback($("#orch2StartDate").val(), $("#orch2EndDate").val(), data);
        });
        this.successCallbackOrch2(start, end, data);
   },
   
   successCallbackOrch2 : function(start, end, plData){
       console.log("here");
       var orch2 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].ORCH2Sold;
            orch2 += parseInt(item);
            counter += 1;
            var cap = counter * 196;
       }
        ...
        function parseDate(start, end){
            ...
            for(var i = 0; i < plData.length; i++){
                var item = plData[i];
                var date = gd(item.EventDate);
                console.log(item.ORCH2Sold);
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 196;
                    return result += parseInt(item.ORCH2Sold);
                }
            }
        }
        ...
        console.log("parsedCap: ", parsedCap);
       ...
       var plotObj = $.plot($("#flot-pie2-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
   ...
   renderOrch3Graph : function(start, end, data) {
        var callback = this.successCallbackOrch3;
        $("#Orch3Form").submit(function(e) {
            e.preventDefault();
            callback($("#orch3StartDate").val(), $("#orch3EndDate").val(), data);
        });
        this.successCallbackOrch3(start, end, data);
   },
   
   successCallbackOrch3 : function(start, end, plData){
       var orch3 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].ORCH3Sold;
            orch3 += parseInt(item);
            counter += 1;
            var cap = counter * 263;
       }
        ...
        function parseDate(start, end){
            ...
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 263;
                    return result += parseInt(item.ORCH3Sold);
                }
            }
        }
       ...
       var plotObj = $.plot($("#flot-pie3-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
   ...
   render4PMEZZGraph : function(start, end, data) {
        var callback = this.successCallback4PMEZZ;
        $("#4PMEZZForm").submit(function(e) {
            e.preventDefault();
            callback($("#4PMEZZStartDate").val(), $("#4PMEZZEndDate").val(), data);
        });
        this.successCallback4PMEZZ(start, end, data);
   },
   
   successCallback4PMEZZ : function(start, end, plData){
       var pmezz4 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].PMEZZ4Sold;
            pmezz4 += parseInt(item);
            counter += 1;
            var cap = counter * 149;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 149;
                    return result += parseInt(item.PMEZZ4Sold);
                }
            }
        }
       ...
       var plotObj = $.plot($("#flot-pie4-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
   ...
   render5FMEZZGraph : function(start, end, data) {
        var callback = this.successCallback5FMEZZ;
        $("#4PMEZZForm").submit(function(e) {
            e.preventDefault();
            callback($("#5FMEZZStartDate").val(), $("#5FMEZZEndDate").val(), data);
        });
        this.successCallback5FMEZZ(start, end, data);
   },
   
   successCallback5FMEZZ : function(start, end, plData){
       var fmezz5 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].FMEZZ5Sold;
            fmezz5 += parseInt(item);
            counter += 1;
            var cap = counter * 174;
       }
       ...
       var plotObj = $.plot($("#flot-pie5-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
    ...
    render6MZBOXGraph : function(start, end, data) {
        var callback = this.successCallback6MZBOX;
        $("#6MZBOXForm").submit(function(e) {
            e.preventDefault();
            callback($("#6MZBOXStartDate").val(), $("#6MZBOXEndDate").val(), data);
        });
        this.successCallback6MZBOX(start, end, data);
   },
   
   successCallback6MZBOX : function(start, end, plData){
       var fmezz5 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].MZBOX6Sold;
            fmezz5 += parseInt(item);
            counter += 1;
            var cap = counter * 8;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 8;
                    return result += parseInt(item.MZBOX6Sold);
                }
            }
        }
        ...
        console.log("MZBOXparsedData: ", parsedData);
        console.log("MZBOXparsedCap: ", parsedCap);
       ...
       var plotObj = $.plot($("#flot-pie6-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
    ...
    render7FMHGraph : function(start, end, data) {
        var callback = this.successCallback7FMH;
        $("#7FMHForm").submit(function(e) {
            e.preventDefault();
            callback($("#7FMHStartDate").val(), $("#7FMHEndDate").val(), data);
        });
        this.successCallback7FMH(start, end, data);
   },
   
   successCallback7FMH : function(start, end, plData){
       var fmezz5 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].FMH7Sold;
            fmezz5 += parseInt(item);
            counter += 1;
            var cap = counter * 62;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 62;
                    return result += parseInt(item.FMH7Sold);
                }
            }
        }
       ...
       var plotObj = $.plot($("#flot-pie7-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
   ...
   render8RMKPGraph : function(start, end, data) {
        var callback = this.successCallback8RMKP;
        $("#8RMKPForm").submit(function(e) {
            e.preventDefault();
            callback($("#8RMKPStartDate").val(), $("#8RMKPEndDate").val(), data);
        });
        this.successCallback8RMKP(start, end, data);
   },
   
   successCallback8RMKP : function(start, end, plData){
       var fmezz5 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].RMKP8Sold;
            fmezz5 += parseInt(item);
            counter += 1;
            var cap = counter * 154;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 154;
                    return result += parseInt(item.RMKP8Sold);
                }
            }
        }
       ...
       var plotObj = $.plot($("#flot-pie8-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
   ...
   render9JOGraph : function(start, end, data) {
        var callback = this.successCallback9JO;
        $("#9JOForm").submit(function(e) {
            e.preventDefault();
            callback($("#9JOStartDate").val(), $("#9JOEndDate").val(), data);
        });
        this.successCallback9JO(start, end, data);
   },
   
   successCallback9JO : function(start, end, plData){
       var fmezz5 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].JO9Sold;
            fmezz5 += parseInt(item);
            counter += 1;
            var cap = counter * 29;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 29;
                    return result += parseInt(item.JO9Sold);
                }
            }
        }
       ...
       var plotObj = $.plot($("#flot-pie9-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
   ...
   render10BALCGraph : function(start, end, data) {
        var callback = this.successCallback10BALC;
        $("#10BALCForm").submit(function(e) {
            e.preventDefault();
            callback($("#10BALCStartDate").val(), $("#10BALCEndDate").val(), data);
        });
        this.successCallback10BALC(start, end, data);
   },
   
   successCallback10BALC : function(start, end, plData){
       var fmezz5 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].BALC10Sold;
            fmezz5 += parseInt(item);
            counter += 1;
            var cap = counter * 304;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 304;
                    return result += parseInt(item.BALC10Sold);
                }
            }
        }
       ...
       var plotObj = $.plot($("#flot-pie10-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
   ...
   render11BLBXGraph : function(start, end, data) {
        var callback = this.successCallback11BLBX;
        $("#11BLBXForm").submit(function(e) {
            e.preventDefault();
            callback($("#11BLBXStartDate").val(), $("#11BLBXEndDate").val(), data);
        });
        this.successCallback11BLBX(start, end, data);
   },
   
   successCallback11BLBX : function(start, end, plData){
       var fmezz5 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].BLBX11Sold;
            fmezz5 += parseInt(item);
            counter += 1;
            var cap = counter * 12;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 12;
                    return result += parseInt(item.BLBX11Sold);
                }
            }
        }
       ...
       var plotObj = $.plot($("#flot-pie11-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
      ...
      render12SROOGraph : function(start, end, data) {
        var callback = this.successCallback12SROO;
        $("#12SROOForm").submit(function(e) {
            e.preventDefault();
            callback($("#12SROOStartDate").val(), $("#12SROOEndDate").val(), data);
        });
        this.successCallback12SROO(start, end, data);
   },
   
   successCallback12SROO : function(start, end, plData){
       var fmezz5 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].SROO12Sold;
            fmezz5 += parseInt(item);
            counter += 1;
            var cap = counter * 10;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 10;
                    return result += parseInt(item.SROO12Sold);
                }
            }
        }
       ...
       var plotObj = $.plot($("#flot-pie12-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
	    ...
	    render13SROMGraph : function(start, end, data) {
        var callback = this.successCallback13SROM;
        $("#13SROMForm").submit(function(e) {
            e.preventDefault();
            callback($("#13SROMStartDate").val(), $("#13SROMEndDate").val(), data);
        });
        this.successCallback13SROM(start, end, data);
   },
   
   successCallback13SROM : function(start, end, plData){
       var fmezz5 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].SROM13Sold;
            fmezz5 += parseInt(item);
            counter += 1;
            var cap = counter * 4;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    var result = 0;
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 4;
                    return result += parseInt(item.SROM13Sold);
                }
            }
        }
       ...
       var plotObj = $.plot($("#flot-pie13-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
   ...
   render14SROBGraph : function(start, end, data) {
        var callback = this.successCallback14SROB;
        $("#14SROBForm").submit(function(e) {
            e.preventDefault();
            callback($("#14SROBStartDate").val(), $("#14SROBEndDate").val(), data);
        });
        this.successCallback14SROB(start, end, data);
   },
   
   successCallback14SROB : function(start, end, plData){
       var fmezz5 = 0;
       var counter = 0;
       
       for(var i = 0; i < plData.length; i++){
            var item = plData[i].SROB14Sold;
            fmezz5 += parseInt(item);
            counter += 1;
            var cap = counter * 8;
       }
        ...
        function parseDate(start, end){
            for(var i = 0; i < plData.length; i++){
                ...
                if ((date >= gd(start)) && (date <= gd(end))){
                    ...
                    parsedCounter += 1;
                    parsedCap = parsedCounter * 8;
                    return result += parseInt(item.SROB14Sold);
                }
            }
        }
       ...
       var plotObj = $.plot($("#flot-pie14-chart #flotcontainerSales"), data, {
           series: {
               pie: {
                   show: true
               }},
